...
public class TestSample3_1extra extends
		ActivityInstrumentationTestCase2<Pattern3TestProjectActivity> {

	public TestSample3_1extra() {
		super(Pattern3TestProjectActivity.class.getPackage().toString(),
				Pattern3TestProjectActivity.class);
	}
		...
		try {
			Pattern3TestProjectActivity.setOnCreate(new OnCreate() {
				...
				public void onCreate(Activity target, Bundle savedInstanceState) {
					...
					int id = info.kyorohiro.helloworld.testsample.extra.R.string.app_name;
//                  following is NotFoundException
//					int id = info.kyorohiro.helloworld.testsample.extra.R.string.option;
			...
			android.util.Log
					.v("testsample", "called testSampleStartActivity()");
			Thread.sleep(3000);
